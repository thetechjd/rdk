...
# Copy the vendored embedding model into the staged dir so electron-builder's
# `extraResources: from build/models` finds it. The model is committed under
# apps/desktop/build/models (a real repo asset — no build-time fetch), and this
# explicit copy makes staging deterministic regardless of pnpm deploy's file
# selection. Without it the packaged app would fall back to a network download.
